...
const configureStatusCollector = async ({
  ...
  const { trigger } = lossyAsync(async () => {
    ...
    await withRetryAsync(async (retryCount) => {
      ...
      const { ok, json } = await fetch(url, { headers: getExtraHeaders ? getExtraHeaders() : {} })
  ...
  const timer = createTimer({ func: trigger, delay })
